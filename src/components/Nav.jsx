    ...
    return (
        <nav className={`items-center gap-2 md:justify-between justify-around w-3/4 md:w-1/2 ${hidden ? "hidden": ""} sm:flex`}>
            <div className="flex flex-col gap-[4vw] my-3 sm:flex-row sm:my-0">
                {tabs.map((tab, index) => (
                    <NavLink key={index} to={tab.to} className="hover:bg-white hover:text-gray-500 sm:hover:text-gray-300 trwi transition-all p-2 rounded-lg sm:hover:bg-inherit text-lg">
                        {tab.name}
                    </NavLink>
                ))}
            </div>
            {
                loggedIn?
                <div className="flex border border-black sm:border-none p-1 rounded sm:flex-col flex-row gap-1 items-center cursor-pointer" onClick={()=>{
