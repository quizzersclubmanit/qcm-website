...
const PlayQuiz = () => {
  ...
  async function submitQuiz(disqualified = false) {
    ...
    try {
      // Final server-side pre-check to avoid duplicates using QuizScore
      const existing = await dbService.select({
        collectionId: "quiz_scores",
        queries: [
          `userId=${data.$id || data.id}`,
          `section=${Number(section)}`
        ]
      ...
      else if (existing && Array.isArray(existing.quiz_scores)) list = existing.quiz_scores

      const already = (list || []).some((e) => Number(e?.section) === Number(section) && (e?.userId === (data.$id || data.id) || e?.userId?.id === (data.$id || data.id) || e?.userId?.$id === (data.$id || data.id)))
      if (already) {
        toast("You've already attempted this quiz section")
        await document.exitFullscreen().catch(() => {})
        navigate("/")
        return
      }
  ...
  useEffect(() => {
    ...
    // Check if user has already attempted the quiz using QuizScore schema
    const checkQuizScore = async () => {
      try {
        ...
        let quizScoreData = []
        if (Array.isArray(docs)) quizScoreData = docs
        else if (docs && Array.isArray(docs.data)) quizScoreData = docs.data
        else if (docs && Array.isArray(docs.quiz_scores)) quizScoreData = docs.quiz_scores

        // Fallback safety filter if server ignored filters
        const filtered = (quizScoreData || []).filter(
          (e) => Number(e?.section) === Number(section) &&
            (e?.userId === userId || e?.userId?.id === userId || e?.userId?.$id === userId)
        )
        ...
        console.error('QuizScore check error:', error)
        // Do not enter fullscreen on error; inform the user instead
        toast("Network error. Please try again.")
        navigate("/")
      } finally {
